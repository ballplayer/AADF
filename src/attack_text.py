...
import numpy as np
from TextFooler import dataloader, criteria
...
def main():
    ...
    message = '{}: original accuracy: {:.3f}%, adv accuracy: {:.3f}%, success rate: {:.3f}%, time(s)/line: {:.3f}, ' \
              'avg changed rate: {:.3f}%, num of queries: {:.1f}\n'.format(args.target_model,
                                                                           (1 - orig_failures / 1000) * 100,
                                                                           (1 - adv_failures / 1000) * 100,
                                                                           (attack_success / predict_success_nums) * 100,
                                                                           attack_time / 1000,
    ...
    with open(os.path.join(args.output_dir, 'result.txt'), 'w') as ofile:
    ...
    print(message, "results saved in: ", args.output_dir)
